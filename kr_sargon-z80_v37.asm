;***********************************************************
; 2021-03-20 v1, HW Kramer
; THIS IS A DEBUG VERSION FOR DETECTING OUT OF RANGE ASSIGNEMENT TO MLPTRJ
; modifications for MPF-IP/IOM-MPF-IP
; the code is reorganized to avoid multple ORG statement
; video memory points to 0E000h
; 2020-05-18 , Bill Foster
; https://github.com/billforsternz/retro-sargon/blob/master/stages/sargon-z80.asm
; 1978 , Dan and Kathe Spracklen
; https://datassette.nyc3.cdn.digitaloceanspaces.com/livros/sargon_a_computer_chess_program.pdf
...
BMOVES:  DB      35,55,10H
        ...
        DB      84,64,10H
;***********************************************************
        DS      START+80H-$     ;this is to align the storage
; ORG   START+80H , commented out as multiple ORG does not work
...
;BLBASE:  EQU     $+512-384     ;this is to align the storage
BLOCK:   EQU     $-BLBASE
...
;***********************************************************
;        ORG     START+2FFH
                DS START+2FFh-$ ;this is to align the storage
STACK:

;***********************************************************
;        ORG     START+300H
                DS START+300H-$ ;;this is to align the storage
MLIST:   DS      2048
...
MVENUM:  DB      "01 "
TITLE3:  DB      "  "
        DB      '[',$2A,']'     ; Part of TITLE 3 - Underlines
        DB      '[',$2A,']'
        DB      '[',$2A,']'
        DB      '[',$2A,']'
        DB      '[',$2A,']'
...
;***********************************************************
;*** move list range check for debugging ***
;modified , 2021-03-20 , hwkramer 
RASSERTB:
        PUSH    AF
        PUSH    DE
        LD              A,B
        CP              0h
        JR              Z,ROKB; this is for the initial phase
        LD              DE,MLIST ; check lower end
        CP              D
        JP              Z,ROKB
        JP              C,RNOKB
        LD              DE,MLEND ; check lower end
        CP              D
        JP              Z,ROKB
        JP              C,ROKB
RNOKB:
        POP     DE
        POP AF
        RST     38h     ;stop and show register
ROKB:
        POP     DE
        POP     AF
...
RASSERTH1:
        ...
        LD              A,H
        CP              0h
        JR              Z,ROKH1; this is for the initial phase
        LD              DE,MLIST ; check lower end
        CP              D
        JR              Z,ROKH1
        JR              C,RNOKH1
        LD              DE,MLEND ; check lower end
        INC             D; takes care for the initial plylist ptr
        CP              D
        JR              Z,ROKH1
        JR              C,ROKH1
RNOKH1:
...
;***********************************************************
        ...
        ;JR      Z,FM25          ; Yes - jump , modified , 2021-03-20 , hwkramer 
        JP      Z,FM25          ; Yes - jump, modified , 2021-03-20 , hwkramer 
...
RASSERTD1:
        ...
        LD              A,D
        CP              0h
        JR              Z,ROKD1; this is for the initial phase
        LD              HL,MLIST ; check lower end
        CP              H
        JR              Z,ROKD1
        JR              C,RNOKD1
        LD              HL,MLEND ; check lower end
        CP              H
        JP              Z,ROKD1
        JR              C,ROKD1
RNOKD1:
...
RASSERTD2:
        ...
        LD              A,D
        CP              0h
        JR              Z,ROKD2; this is for the initial phase
        LD              HL,MLIST ; check lower end
        CP              H
        JR              Z,ROKD2
        JR              C,RNOKD2
...
;        DW      NAME,LNGTH
        PUSH    BC
        PUSH    HL
        LD              B,LNGTH         ;the length of the message
        LD              HL,NAME         ;the pointer of the message
        CALL    MPRTLIN         ;insert MPF code
        POP HL
        POP BC
        ENDM
...
DRIVER: ;LD      sp,STACK        ; Set stack pointer, TODO?
                LD sp, 0EFFFH                   ; TODO MPF this experimental
                ;LD sp, 0DFFFH                  ; TODO MPF this experimental
                ;CALL INIUART                   ; initialite the UART of the MPF-IP-IOM
...
DRIV04: PRTBLK  MVENUM, 3        ; Display move number
        ...
        CARRET                  ; New line
        ;CALL PRTBRD                            ; MPF specific , 2021-03-15 , hwkramer , disabled
        JR      DR0C            ; Jump
...
RASSERTH2:
        ...
        LD              A,H
        CP              0h
        JR              Z,ROKH2; this is for the initial phase
        LD              DE,START ;this is for the initial best move, fom the book
        CP              D
        JR              Z,ROKH2
        LD              DE,MLIST ; check lower end
        CP              D
...
; ARGUMENTS:  --  None
;***********************************************************
PLYRMV: CALL    INIUART                 ; MPF, just to make sure 
...
RASSERTIX:
        ...
        LD              A,D
        CP              0h
        JR              Z,ROKIX; this is for the initial phase
        LD              DE,MLIST ; check lower end
        CP              D
        JR              Z,ROKIX
        JR              C,RNOKIX
...
RASSERTH3:
        ...
        LD              A,H
        CP              0h
        JR              Z,ROKH3; this is for the initial phase
        LD              DE,MLIST ; check lower end
        CP              D
        JR              Z,ROKH3
        JR              C,RNOKH3
...
CHARTR: ;RST     38h             ; Jove monitor single char inpt
        ;DB      81H,0
        CALL    MCHRRD                  ; MPF-IP-IOM single char inpt
        ...
        ;DB      81H,1AH
        CALL    MCHROUT                 ; MPF-IP-IOM single char echo
...
;                 be marked.
;***********************************************************
DSPBRD: RET             ; modified 2021-03-15 , hwkramer 
...
;                 pair HL.
;***********************************************************
INSPCE: RET             ; modified 2021-03-15 , hwkramer 
...
;                 HL.
;***********************************************************
CONVRT: RET     ; modified, 2021-03-18 , hwkramer 
                PUSH    bc              ; Save registers
        PUSH    de
        PUSH    af
        LD      a,(BRDPOS)      ; Get board index
...
;        ADD     a,0C0H          ; Rank Norm address
        ADD     a,0E0H          ; Rank Norm address, this for MPF
...
;                 register B.
;***********************************************************
BLNKER: RET     ; modified 2021-03-15, hwkramer 
...
;***********************************************************
MAKEMV: RET     ; modified 2021-03-18, hwkramer
...
;***********************************************************
; I/O routines for MPF-IP/IOM-MPF-IP
;***********************************************************
;IOM-MPF-IP Addresses
;assumes the IOM-MPF-IP card is fitted. The UART is an 8251AF and the CTC
;is the standard Z80 type.
URTDA:  EQU     60H;USART DATA
URTCNT: EQU     61H;USART CONTROL
CTC2:   EQU     66H;BAUD RATE GENERATOR
;***********************************************************
;***********************************************************
;
...
INIUART:
        ...
        PUSH    BC
        PUSH    HL
        LD      A,47h           ;counter mode, time constant follows, reset channel
...
RESTAB: 
        ...
        DEFB    00h
        DEFB    00h
        DEFB    40h     ;internal reset
        DEFB    4Eh     ;baud rate factor 16, no parity check, 1 stop bit
        DEFB    37h     ;/RTS=0, reset error flag, normal operation, receive enable, /DTR=0, transmit enable
TABEND:
;
;***********************************************************
...
;*** PRINT ANY LINE (NAME, LENGTH) ***
; INPUT:        HL points to the text buffer
;                       B is the lenght of the string
;
; assume that RS232 ist initialized
;
MPRTLIN:
        PUSH    AF
        CALL INIUART
...
MEXIT: 
        RST 38h
;delay routine
;2021-03-18 modify delay, less risky
; see Leventhal, Z80 Assenly Language Programming, 11-10
; Number of milliseconds in Register B
MSCNT:          EQU     070h; COUNT FOR 1 MSEC AT 1.79 MHZ Z80 
MSTIME:         EQU             0FAh; this is 250 Milliseconds
DELAY:
        LD      C,MSCNT
DLY1:   DEC     C
                JR NZ,DLY1
...
WAIT1:  IN      A,(URTCNT)
        ...
        RET
        ;
;INPUT A CHARACTER FROM USART
;
MCHRRD:
        IN      A,(URTCNT)              ;Get status byte
        BIT     1,A                             ;Check buffer full bit
        JR      Z,MCHRRD                ;Not full so wait
...
;*** CONVERT BOARD CODE TO CHESS PIECE ***
; INPUT: Reg A with board code
; OUTPUT: Reg A with print character
; Register affected: AF
CONVERTB:
        CP      0FFh
        JR NZ,FIGUR
        LD      A,'x' ; print for outside the board
        RET
FIGUR:
        PUSH BC
        PUSH HL
...
CONT:
        ...
        RET     
WFIGUR:
        DEFM ".PNBRQKE" ; E means error, this should not occur
BFIGUR:
        DEFM ".pnbrqke" ; e means error, this should not occur
        
;this is to print a board
PRTBRD:
        PUSH    AF      ;save registers
        PUSH    BC
        PUSH    DE
...
;outer loop
        LD      C,0
        LD      B,12    ;our board has 12 ranks 
        LD      D,0
        LD      E,120
        ADD HL,DE       
PRTRNK:                 ;print one rank
        PUSH    BC
;inner loop
        LD      C,0
        LD      B,10    ;our board has 10 files
        LD      D,0
        LD      E,10
...
PRTFIL: 
        LD      A,(HL)
        INC     HL
        CALL CONVERTB   ;convert from figure code to figure
        CALL MCHROUT    ;output to console
        DJNZ    PRTFIL  ;inner loop
        LD      A, 0AH          ;make a line feed
        CALL MCHROUT    ;output to console
        LD      A, 0DH          ;make a cr feed
        CALL MCHROUT    ;output to console
        POP     HL
        POP     BC
        DJNZ PRTRNK             ;outer loop
        LD      A, 0AH          ;make a line feed
        CALL MCHROUT    ;output to console
        LD      A, 0DH          ;make a cr feed
        CALL MCHROUT    ;output to console
